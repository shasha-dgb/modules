...
@loader.tds
class LQuotes(loader.Module):
    """
    Quotes By LukaszMods
    """

    strings = {
        "name": "LQuotes",
        "no_reply": "<b>[LQuotes]</b> Нет реплая",
        "processing": "<b>[LQuotes]</b> Обработка...",
        "api_processing": "<b>[LQuotes]</b> Ожидание API...",
        "api_error": "<b>[LQuotes]</b> Ошибка API",
        "loading_media": "<b>[LQuotes]</b> Отправка...",
        "no_args_or_reply": "<b>[LQuotes]</b> Нет аргументов или реплая",
        "args_error": "<b>[LQuotes]</b> При обработке аргументов произошла ошибка. Запрос был: <code>{}</code>",
        "too_many_messages": "<b>[LQuotes]</b> Слишком много сообщений. Максимум: <code>{}</code>"
    }
    ...
    async def qcmd(self, message: types.Message):
        """
        Сокращение команды .lq
        """

        return await self.lqcmd(message)


    async def lqcmd(self, message: Message):
        """
        Использование:
    ...
    async def flqcmd(self, message: Message):
        """
        Использование:

        • .flq <@ или ID> + <текст> - квота от юзера с @ или ID + указанный текст
        >>> .flq @onetimeusername Вам пизда

        • .fsq <реплай> + <текст> - квота от юзера с реплая + указанный текст
        >>> .flq Я лох

        • .flq <@ или ID> + <текст> + -r + <@ или ID> + <текст> - квота с фейковым реплаем
        >>> .flq @Fl1yd спасибо -r @onetimeusername Ты крутой

        • .flq <@ или ID> + <текст> + -r + <@ или ID> + <текст>; <аргументы> - квота с фейковыми мульти сообщениями
        >>> .flq @onetimeusername Пацаны из @sh1tchannel, ждите награду за ахуенный ботнет; @elonmuskplssuckmybigdick чево; @Fl1yd НАШ БОТНЕТ ЛУЧШИЙ -r @elonmuskplssuckmybigdick чево
        """
